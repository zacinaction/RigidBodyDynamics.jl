...
tree_joints(m::Mechanism) = (edge_to_parent_data(vertex) for vertex in non_root_vertices(m))
Base.@deprecate joints(m::Mechanism) tree_joints(m)
...
num_positions(m::Mechanism) = num_positions(tree_joints(m))
num_velocities(m::Mechanism) = num_velocities(tree_joints(m))
